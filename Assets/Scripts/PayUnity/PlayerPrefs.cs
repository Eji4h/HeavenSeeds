...
namespace PreviewLabs
{
	public static class PlayerPrefs
	{
...
#if UNITY_WEBPLAYER
        private static byte[] bytes = Encoding.UTF8.GetBytes("13ex" + SystemInfo.deviceUniqueIdentifier.Substring(0, 4));
#else
        private static byte[] bytes = Encoding.UTF8.GetBytes("13ex" + 
            SystemInfo.deviceUniqueIdentifier.Substring(0, 4) + 
            GetProjectName().Substring(0, 4) + "Payz");
#endif
...
#if UNITY_WEBPLAYER
            cryptor = new DESCryptoServiceProvider().CreateEncryptor(bytes, bytes);
#else
            cryptor = new AesManaged().CreateEncryptor(bytes, bytes);
#endif
		...
		private static string Decrypt (string cryptedString)
		{
            ...
            ICryptoTransform cryptor;
#if UNITY_WEBPLAYER
            cryptor = new DESCryptoServiceProvider().CreateDecryptor(bytes, bytes);
#else
            cryptor = new AesManaged().CreateDecryptor(bytes, bytes);
#endif
